...
namespace MSRApi.WEBBL
{
    public class WebBL
    {
        ...
        public async Task<List<PrivateTaskModel>> GetWebPrivateData(int Month, int Year)
        {
            ...
            using (var ctx = new DbEntities())
            {
                var webData = await ctx.Tasks.Where(x => x.MonthId == Month && x.Year == Year && x.Type == 2 && x.IsActive == true).Include(x => x.SubTasks).ThenInclude(xy => xy.JiraTickets).ToListAsync();
        ...
        public async Task<ResponceModel> AddTaskAndSubTask(TaskAndSubTaskModel taskMst)
        {
            ...
            try
            {
                ...
                if(ifexist == null)
                {
                    ...
                    JiraTicket jiratickets = new JiraTicket();
                    jiratickets.SubTaskId = subtask.Id;
                    jiratickets.JiraTicketId = taskMst.JiraTicketId;
                    if (jiratickets.SubTaskId != 0 && jiratickets.JiraTicketId != 0)
                    {
                        using (var ctx = new DbEntities())
                        {
                            await ctx.JiraTickets.AddAsync(jiratickets);
                            await ctx.SaveChangesAsync();
                        }
                    }
